 ...
 * @hide
 */
class NotificationColorUtil private constructor(context: Context) {
    private val mGrayscaleBitmapCache = WeakHashMap<Bitmap, Pair<Boolean, Int>>()
    private val mGrayscaleIconMaxSize // @dimen/notification_large_icon_width (64dp)
            : Int
    ...
    private object ColorUtilsFromCompat {
        ...
        fun RGBToLAB(
            ...
            XYZToLAB(
                outLab[0], outLab[1], outLab[2], outLab
            )
        ...
        fun XYZToColor(
            ...
            return Color.rgb(
                constrain(
                    (r * 255).roundToInt(), 0, 255
                ),
                constrain(
                    (g * 255).roundToInt(), 0, 255
                ),
                constrain(
                    Math.round(b * 255).toInt(), 0, 255
                )
            )
        }
    ...
    companion object {
        private val TAG = "NotificationColorUtil"
        ...
        fun clearColorSpans(charSequence: CharSequence): CharSequence {
            if (charSequence is Spanned) {
                ...
                val builder = SpannableStringBuilder(ss.toString())
                for (span: Any in spans) {
                    var resultSpan: Any? = span
                    ...
                    builder.setSpan(
                        resultSpan, ss.getSpanStart(span), ss.getSpanEnd(span),
                        ss.getSpanFlags(span)
                    )
                }
                return builder
        ...
        fun changeColorLightness(baseColor: Int, amount: Int): Int {
            ...
            result[0] = max(min(100.0, result[0] + amount), 0.0)
            return ColorUtilsFromCompat.LABToColor(
                result[0], result[1], result[2]
            )
        }
    ...
    init {
        mGrayscaleIconMaxSize = context.resources.getDimensionPixelSize(
